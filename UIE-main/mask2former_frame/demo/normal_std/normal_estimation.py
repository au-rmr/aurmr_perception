...
if __name__ == "__main__":
    ...
    depth_file = os.path.join(dataset_root, 'scenes', 'scene_%04d' %
                              scene_idx, camera, 'depth', '%04d.png' % anno_idx)
    meta_file = os.path.join(dataset_root, 'scenes', 'scene_%04d' %
                             scene_idx, camera, 'meta', '%04d.mat' % anno_idx)
    segmask_file = os.path.join(dataset_root, 'scenes', 'scene_%04d' %
                                scene_idx, camera, 'label', '%04d.png' % anno_idx)
    ...
    pc_sampled.estimate_normals(o3d.geometry.KDTreeSearchParamRadius(
        0.012), fast_normal_computation=False)
    ...
    for idx in range(len(points_sampled)):
        ...
        arrow_points = np.dot(R, arrow_points.T).T + \
            suction_point[np.newaxis, :]
        arrow.vertices = o3d.utility.Vector3dVector(arrow_points)
        arrow_list.append(arrow)

    o3d.visualization.draw_geometries(
        [pc_o3d, *arrow_list], width=1280, height=720)
