...
class binDataset(Dataset):
    def __init__(self, file_list, for_training=False, crop=False):
...
class binFrameDataset(Dataset):
    ...
    def __getitem__(self, index):
        # For now, we only care about the more packed frame
        sample_two_framed = {}
        for frame_n in range(2):
            cat = f'frame{frame_n}_'
            ...
            # Adds noise and cuts out random ellipses from depth
            if self.for_training:
                depth = self.add_noise_to_depth(self.dropout_random_ellipses(depth, data_loading_params),
                                                data_loading_params)
            ...
            # Randomly crop the image, point cloud, and label
            if self.for_training:
                rgb, label, xyz = self.random_crop(rgb, label, xyz)
            ...
            if self.for_training:
                ...
                sample['label'] = label_blob.unsqueeze(0)
            sample_two_framed[f'frame{frame_n}'] = sample
        return sample_two_framed
