...
class FrameMaskFormerV2(nn.Module):
    """
    ...
    def forward(self, batched_sequences):
        ...
        batched_outputs = []
        batched_images = []
        if type(batched_sequences[0]['image']) == list:
            num_frames = len(batched_sequences[0]['image'])
            for frame_idx in range(num_frames):
                images = [b["image"][frame_idx].to(
                    self.device) for b in batched_sequences]
                images = [(x - self.pixel_mean) /
                          self.pixel_std for x in images]
                images = ImageList.from_tensors(images, self.size_divisibility)
                batched_images.append(images)
        else:
            num_frames = 1
            images = [b["image"].to(self.device) for b in batched_sequences]
        ...
        if self.training:
            ...
            for f in range(num_frames):
                ...
                for cur_batch in batched_sequences:
                    ...
                    cur_batch['instances'][f].set(
                        "track_id", cur_batch['instances'][f].gt_ids)
        ...
        if self.training:
                    ...
                    for b, t in enumerate(batched_targets[frame_id+1]):
                        ...
                        for track_id in track_id_cur_frame:
                            if track_id in track_id_prev_frame:
                                track_index_in_prev_frame = track_id_prev_frame.index(
                                    track_id)
                                src_idx = indices[b][0].tolist()
                                tgt_idx = indices[b][1].tolist()
                                query_index_in_prev_frame = src_idx[tgt_idx.index(
                                    track_index_in_prev_frame)]
                                gt_matched_history_indices.append(
                                    query_index_in_prev_frame)
                            else:
                        ...
                        if self.reid_loss_over == 'all':
                            ...
                            for i in gt_matched_history_indices:
                                if i != -1:
                                    track_id.append(
                                        batched_outputs[frame_id]["pred_reid"][b][i])
                                    gt_matched_track_indices.append(
                                        len(track_id)-1)
                                else:
                                    gt_matched_track_indices.append(-1)
                            history[b]["track_reid"] = torch.stack(
                                track_id, dim=0)
                            history[b]['gt_matched_track_indices'] = torch.as_tensor(
                                gt_matched_track_indices)
            ...
            for frame_id in range(num_frames):
                ...
                for batch_idx, mask_cls_result, mask_pred_result, reid_embed_result, input_image, image_size in zip(
                    range(
                        batch_size), mask_cls_results, mask_pred_results, reid_embed_results, batched_images, images.image_sizes
                ):
                    # adapted to new data format
                    height = batched_sequence_in_frames[batch_idx].get(
                        "height", image_size[0])
                    width = batched_sequence_in_frames[batch_idx].get(
                        "width", image_size[1])
                    processed_result = {}
                    ...
                    if self.instance_on:
                        instance_r = retry_if_cuda_oom(self.instance_inference)(
                            mask_cls_result, mask_pred_result, reid_embed_result)
                        processed_result["instances"] = instance_r

                    all_frame_processed_results[batch_idx].append(
                        processed_result)

            if num_frames == 1:
                all_frame_processed_results = [x[0]
                                               for x in all_frame_processed_results]
                return all_frame_processed_results
            else:
                instances = self.postprocess_video_output(
                    all_frame_processed_results)
    ...
    def postprocess_video_output(self, all_frame_processed_results):
        ...
        """
        assert len(
            all_frame_processed_results) == 1, "Only support single video inference"
        method = 'hungarian'
        # topk_sequence = heapq()
        for batch_idx, batch_results in enumerate(all_frame_processed_results):
            ...
            for frame_idx, frame_results in enumerate(batch_results):
                ...
                if len(instances_in_history) == 0 and len(instances_cur_frame) > 0:
                    instances_cur_frame = [
                        {k: [v] for k, v in instances.items()} for instances in instances_cur_frame]
                    instances_in_history = instances_cur_frame
                    continue

                if method == 'hungarian':
                    # find the best match for each instance in the current frame
                    reid_embed_in_history = torch.stack(
                        [x['reid_embed'][-1] for x in instances_in_history])
                    reid_embed_in_cur_frame = torch.stack(
                        [x['reid_embed'] for x in instances_cur_frame])
                    similarity_matrix = torch.einsum(
                        "nc,mc->nm", reid_embed_in_history.to(torch.float), reid_embed_in_cur_frame.to(torch.float))
                    ...
                    indices_in_prev_frame, indices_in_cur_frame = linear_sum_assignment(
                        -similarity_matrix)
                    ...
                    for instance_idx_in_prev_frame, instance in enumerate(instances_in_history):
                        if instance_idx_in_prev_frame in indices_in_prev_frame:
                            # update the instance
                            instance_idx_in_cur_frame = indices_in_cur_frame[
                                indices_in_prev_frame == instance_idx_in_prev_frame][0]
                            matched_instance_indices.append(
                                instance_idx_in_cur_frame)
                    ...
                    reid_embed_in_cur_frame = torch.stack(
                        [x['reid_embed'] for x in instances_cur_frame])
                    similarity_matrix = torch.einsum(
                        "nc,mc->nm", reid_embed_in_history, reid_embed_in_cur_frame)
                    ...
                    for instance_idx_in_prev_frame, instance_sequence in enumerate(instances_in_history):
                        for instance_idx, instance_to_update in enumerate(instances_cur_frame):
                            instance_to_update = {
                                k: v for k, v in instance_to_update.items()}
                            instance_to_update['scores'] = instance_to_update['scores']*instance_sequence['scores'][-1] * \
                                similarity_matrix[instance_idx_in_prev_frame][instance_idx].item(
                            )

                            instance_history_copy = {
                                k: [t for t in v] for k, v in instance_sequence.items()}
                            for k in instance_history_copy.keys():
                                instance_history_copy[k].append(
                                    instance_to_update[k])
                            new_history.append(instance_history_copy)
                    instances_in_history = new_history
                    # add a new instance
                    for instance_idx, instance in enumerate(instances_cur_frame):
                        new_instance_this_frame = {k: [v]
                                                   for k, v in instance.items()}
                        instances_in_history.append(new_instance_this_frame)
        if method == 'beam_search':
            instances_in_history = self.use_prior_in_amazon_task(
                instances_in_history)
    ...
    def use_prior_in_amazon_task(self, instances):
        ...
        for instance_idx, instance in enumerate(instances):
            ...
            print(
                f"{instance_idx}, {instance['idx_in_img']}, {instance['scores'][-1]}, {max_score[object_idx, object_life_length]}")
        ...
        for i in range(num_object):
            for j in range(max_life_length):
                print(
                    f"{i}, {j}, {max_score[i, j]}, {max_score_idx[i, j]}, {instances[max_score_idx[i, j]]['idx_in_img']}")
        # remove sequence with length 0, 1 and max_life_length
        valid_score = max_score[:, 2:-1]
        object_idx, life_time = linear_sum_assignment(-valid_score)
        instance_indices = [max_score_idx[object_idx[i],
                                          life_time[i]+2] for i in range(len(object_idx))]
    ...
    def convert_to_ytvis_format(self, instances):
        ...
        :return: list of dict
        """
        results_ytvis = {"pred_scores": [], "pred_labels": [], "pred_masks": [
        ], "image_size": instances[0]['pred_masks'][0].shape}
    ...
    def instance_inference(self, mask_cls, mask_pred, reid_embed=None):
        ...
        topk_indices = torch.div(
            topk_indices, self.sem_seg_head.num_classes, rounding_mode='floor')
