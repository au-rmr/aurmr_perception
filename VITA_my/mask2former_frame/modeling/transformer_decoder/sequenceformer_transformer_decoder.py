...
class AssociatorDecoder(nn.Module):
    def __init__(
        ...
        self.transformer_asso_cross_layers = nn.ModuleList()
        self.transformer_asso_self_layers = nn.ModuleList()
        self.transformer_asso_ffn_layers = nn.ModuleList()
        for i in dec_layers:
            self.transformer_asso_cross_layers.append(
                CrossAttentionLayer(
                    d_model=hidden_dim,
                    nhead=nheads,
                    dropout=0.0,
            ...
            self.transformer_asso_self_layers.append(
                SelfAttentionLayer(
                    d_model=hidden_dim,
                    nhead=nheads,
                    dropout=0.0,
            ...
            self.transformer_asso_ffn_layers.append(
                FFNLayer(
                    d_model=hidden_dim,
    ...
    def forward(self, src, src_pos, i):
...
class SequenceMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, frame_memory=None, mask = None):
        ...
        for frame_idx in range(num_frames):
            out.append({
                'pred_logits': predictions_class[-1][frame_idx],
                'pred_masks': predictions_mask[-1][frame_idx],
                'pred_reid': predictions_reid[-1][frame_idx],
