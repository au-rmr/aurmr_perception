...
from vita.data.datasets.amazon import COCO_TO_AMAZON
    ...
    def __init__(
        ...
        *,
        ...
        if not is_tgt:
            ...
            elif tgt_dataset_name.startswith("amazon"):
                src2tgt = COCO_TO_AMAZON
            else:
                raise NotImplementedError
...
class AmazonSynV2Mapper:
    ...
    def read_image(self, dataset_dict, frame_idx, format="BGR"):
        if 'preload_image' in dataset_dict:
            ...
            scene_id = int(v[1:])
            frame_id = int(f[1:])
            image = self.h5_file[f'frame{frame_id}_{type}'][scene_id][:, :, :3]
            if format == "BGR":
                image = image[:, :, ::-1]
            elif format == "RGB": 
                pass
