...
class Vita(nn.Module):
    ...
    def prepare_targets(self, targets, images):
        ...
        for targets_per_video in targets:
            ...
            # NOTE: amazon don't have video len
            video_len = targets_per_video["video_len"] if "video_len" in targets_per_video else len(targets_per_video['image'])
            frame_idx = targets_per_video["frame_idx"] if "frame_idx" in targets_per_video else torch.arange(video_len)
            
            clip_gt_instances.append(
                {
                    "labels": gt_classes_per_video, "ids": gt_ids_per_video, "masks": gt_masks_per_video,
                    "video_len": video_len, "frame_idx": frame_idx,
                }
            )
