...
# copied from criterion, to implement contrastive loss
"""
...
class SetCriterionV3(nn.Module):
    ...
    def loss_reid(self, outputs, targets, indices, matching_table, ref_outputs=None, ref_indices=None, ref_matching_table=None):
        loss_reid = 0
        loss_inv_reid = 0
        ...
        for b in range(batch_size):
            ...
            if self.reid_loss_over == "matched":
                pred_reid = [pred_reid[frame_idx][query_idx] for frame_idx, (query_idx, gt_idx) in enumerate([x[b] for x in indices])]
                pred_match = [pred_match[frame_idx][query_idx] for frame_idx, (query_idx, gt_idx) in enumerate([x[b] for x in indices])]
                ref_reid = [ref_reid[frame_idx][query_idx] for frame_idx, (query_idx, gt_idx) in enumerate([x[b] for x in ref_indices])]
                ref_match = [ref_match[frame_idx][query_idx] for frame_idx, (query_idx, gt_idx) in enumerate([x[b] for x in ref_indices])]
            else:
                pred_reid = [x for x in pred_reid]
                pred_match = [x for x in pred_match]
                ref_reid = [x for x in ref_reid]
                ref_match = [x for x in ref_match]
            ...
            if loss_type == 'contrastive':
                if metric=='cosdist':
                    pred_reid_flatten = torch.concat(pred_reid, dim=0)
                    ref_reid_flatten = torch.cat([ref_reid[i] for i in range(num_frames)], dim=0)
                    pred_match_flatten = torch.concat(pred_match, dim=0).float().detach()
                    ...
                    d = 0.5*(1-cosine_sim)
                    K = max(matching_table[b].shape[-1], 2)
                    alpha=0.02
                    sigma=0.5
                ...
                for frame_idx in range(num_frames):
                    ...
                    if other_match.shape[0] != 0 and cur_match.shape[0] != 0:
                        ...
                        if match_label.max() < 0:
                            ...
                            continue
                        loss_reid += F.cross_entropy(similarity_map*logit_scale, match_label)
                        ...
                        match_value, match_label = torch.max(match_map, dim=1)
                        match_label[match_value==0] = -100
                        loss_inv_reid += F.cross_entropy(self_similarity_map*logit_scale, match_label)
                        # if torch.isnan(loss_reid):
                        #     print(match_map.shape, match_label.shape, similarity_map.shape, match_label)
                    else:
        ...
        return {'loss_reid': loss_reid/batch_size,
                'loss_inv_reid': loss_inv_reid/batch_size}
    ...
    def forward(self, outputs, targets):
        ...
        for frame_idx in range(num_frames):
            ...
            indices_last_layer.append(indices)
        ...
        if 'reid' in self.losses:
            matching_table_last_layer = self.create_matching_table(outputs, targets, indices_last_layer)
            losses.update(self.loss_reid(outputs, targets, indices_last_layer, matching_table_last_layer))
            ...
            for i in range(num_aux):
                ...
                if 'reid' in self.losses:
                    if outputs_current_layer[0]['pred_reid'] == None or outputs_current_layer[1]['pred_reid'] == None:
                        continue
                    matching_table = self.create_matching_table(outputs_current_layer, targets, indices_all)
                    l_dict = self.loss_reid(outputs_current_layer, targets, indices_all, matching_table, outputs, indices_last_layer, matching_table_last_layer)
                    l_dict = {k + f"_{i}": v for k, v in l_dict.items()}
                    losses.update(l_dict)
                
        return losses, indices_all
