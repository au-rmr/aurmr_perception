...
def grid_sample(pred_score_map, down_rate=20, topk=50):
...
def inference():
    for anno_idx in range(1):
        ...
        segmask_file = "/home/soofiyanatar/Documents/AmazonHUB/UIE-main/masks/label1.png"
        depth_file = "/home/soofiyanatar/datasets/Full_Dataset/depth_1.png"
        rgb_file = "/home/soofiyanatar/Documents/AmazonHUB/UIE-main/annotated_real_v1_resized/images/scene_03/bin_1E/bin_1E_color_0007.png"
        ...
        depth = cv2.imread(depth_file, cv2.IMREAD_UNCHANGED).astype(
            np.float32)/1000.0
        seg_mask = cv2.imread(
            segmask_file, cv2.IMREAD_UNCHANGED)
        ...
        binary_mask.save(save_root+'/%04d_example' % anno_idx+'.png')
        ...
        suction_scores, idx0, idx1 = grid_sample(
        ...
        if(depth[(idx0[suction_number]), int(idx1[suction_number])] < 1.2):
            drawGaussian(score_image, [
                idx1[suction_number], idx0[suction_number]], 1.0, 3)
            drawGaussian(score_image, [int(center[0]), int(center[1])], 0.5, 3)
