...
# First Party
# visii_tools
import visii_tools.render
from visii_tools.render import render_utils
from visii_tools.utils import mat_to_numpy, numpy_to_mat4, vec_to_numpy, \
    visii_camera_frame_to_rdf, visii_transform_to_dict
...
def add_random_obj(name="name",
    ...
    if obj_id == 2:
        if add_random_obj.create_teapotahedron is None:
            add_random_obj.create_teapotahedron = visii.mesh.create_teapotahedron(
                name)
    ...
    if obj_id == 4:
        if add_random_obj.create_capped_cone is None:
            add_random_obj.create_capped_cone = visii.mesh.create_capped_cone(
                name)
        mesh = add_random_obj.create_capped_cone
    if obj_id == 5:
        if add_random_obj.create_capped_cylinder is None:
            add_random_obj.create_capped_cylinder = visii.mesh.create_capped_cylinder(
                name)
    ...
    if obj_id == 9:
        if add_random_obj.create_dodecahedron is None:
            add_random_obj.create_dodecahedron = visii.mesh.create_dodecahedron(
                name)
        mesh = add_random_obj.create_dodecahedron
    if obj_id == 10:
        if add_random_obj.create_icosahedron is None:
            add_random_obj.create_icosahedron = visii.mesh.create_icosahedron(
                name)
    ...
    if obj_id == 12:
        if add_random_obj.create_rounded_box is None:
            add_random_obj.create_rounded_box = visii.mesh.create_rounded_box(
                name)
...
def random_material(
    ...
    if random_material.textures is None:
        ...
        for folder in glob.glob(path + "/*/"):
            for folder_in in glob.glob(folder + "/*/"):
                name = folder_in.replace(folder, '').replace(
                    '/', '').replace('\\', '')
                # print (folder_in)
                # print (name)
    ...
    texture = random_material.textures[random.randint(
        0, len(random_material.textures) - 1)]
    ...
    if random.uniform(0, 1) < 0.25 or just_simple is True:
        ...
        if 'transmisive_roughness' in texture.keys():
            obj_mat.set_transmission_roughness(
                texture['transmisive_roughness'])
        ...
        if tex is None:
            tex = visii.texture.create_from_image(
            ...
            tex_r = visii.texture.create_from_data(texture['base'] + '_r',
                                                   im_r.shape[0],
                                                   im_r.shape[1],
                                                   im_r.reshape(im_r.shape[0] * im_r.shape[1],
                                                                4).astype(np.float32).flatten().tolist()
                                                   )
            ...
            tex_m = visii.texture.create_from_image(
                texture['base'] + '_m', "tmp.png")
...
def random_translation(obj_id,
    ...
    """
    Randomly moves object around. If it gets close enough to the goal
    sample a new "destination" and go there
    """
    # return
    ...
    if not str(obj_id) in random_translation.destinations.keys():
        ...
        random_translation.speeds[str(obj_id)] = random.uniform(
            speed_lim[0], speed_lim[1])
    else:
        ...
        if motion_blur:
            trans.set_linear_velocity(
                visii.vec3(
                    ...
                    dir_vec[0] *
                    random_translation.speeds[str(obj_id)] * strenght,
                    dir_vec[1] *
                    random_translation.speeds[str(obj_id)] * strenght,
                    dir_vec[2] *
                    random_translation.speeds[str(obj_id)] * strenght
...
circle_path.locations = [0]


def random_sample_hemisphere(
    max_radius=2.5,
    min_radius=0.2
):
    if min_radius > max_radius:
        print('min bigger than max')
        raise('min bigger than max')
    outside = True
    while outside:
        ...
        if (x**2 + y**2 + z**2) * max_radius < max_radius \
            ...
            outside = False
    random_sample_hemisphere.locations = [
        x * max_radius, y * max_radius, z * max_radius]
    return [x * max_radius, y * max_radius, z * max_radius]


random_sample_hemisphere.locations = [0]
...
def random_rotation(obj_id,
    ...
    if not str(obj_id) in random_rotation.destinations.keys():
        ...
        random_rotation.speeds[str(obj_id)] = random.uniform(
            speed_lim[0], speed_lim[1])
        ...
        if Quaternion.sym_distance(goal, rot) < 0.1:
            ...
            random_rotation.speeds[str(obj_id)] = random.uniform(
                speed_lim[0], speed_lim[1])
            goal = random_rotation.destinations[str(obj_id)]
        dir_vec = Quaternion.slerp(
            rot, goal, random_rotation.speeds[str(obj_id)])
        q = visii.quat()
...
def random_scale(obj_id,
    ...
    if not str(obj_id) in random_scale.destinations.keys():
        if not x_lim is None:
            ...
            random_scale.destinations[str(obj_id)] = random.uniform(
                scale_lim[0], scale_lim[1])

        random_scale.speeds[str(obj_id)] = random.uniform(
            speed_lim[0], speed_lim[1])
        ...
        if x_lim is None:
            ...
            for i in range(3):
                if abs(goal[i] - current[i]) < limit[i]:
                    random_scale.destinations[str(obj_id)][i] = random.uniform(
...
def random_color(obj_id,
    ...
    if not str(obj_id) in random_color.destinations.keys():
        c = eval(str(random_color.rcolor.generate(
            luminosity='bright', format_='rgb')[0])[3:])
        random_color.destinations[str(obj_id)] = visii.vec3(
            c[0] / 255.0, c[1] / 255.0, c[2] / 255.0)
        random_color.speeds[str(obj_id)] = random.uniform(
            speed_lim[0], speed_lim[1])
...
def random_light(obj_id,
    ...
    obj.get_light().set_intensity(
        random.uniform(intensity_lim[0], intensity_lim[1]))
    obj.get_light().set_exposure(
        random.uniform(exposure_lim[0], exposure_lim[1]))
    ...
    if not color is None:
        ...
        obj.get_light().set_temperature(random.uniform(
            temperature_lim[0], temperature_lim[1]))
...
def random_intensity(obj_id,
    ...
    if not str(obj_id) in random_intensity.destinations.keys():
        random_intensity.destinations[str(obj_id)] = random.uniform(
            intensity_lim[0], intensity_lim[1])
        random_intensity.speeds[str(obj_id)] = random.uniform(
            speed_lim[0], speed_lim[1])
        random_intensity.current[str(
            obj_id)] = random_intensity.destinations[str(obj_id)]
        ...
        random_intensity.current[str(
            obj_id)] = random_intensity.current[str(obj_id)] + q
...
def random_texture_material(entity):
    ...
    texture = random_texture_material.textures[random.randint(
        0, len(textures) - 1)]
...
def export_to_ndds_file(
    ...
    # record some additional camera information
    T_world_Cv = visii_transform_to_dict(visii.entity.get(
        camera_name).get_transform())['mat']
    T_world_camera = visii_camera_frame_to_rdf(T_world_Cv)
    dict_out['camera_data']['world_t_camera'] = T_world_camera.tolist()
    ...
    for obj_name in obj_names:
        ...
        world_t_object = visii_transform_to_dict(trans)['mat']
        ...
        if segmentation_mask is None:
            segmentation_mask = render_utils.render_segmentation(width, height)

        if visibility_percentage == True and int(
                id_keys_map[obj_name]) in np.unique(segmentation_mask.astype(int)):
            transforms_to_keep = {}
            ...
            visibility = len(values_segmentation) / \
                float(len(values_segmentation_full))
            ...
            if int(id_keys_map[obj_name]) in np.unique(segmentation_mask.astype(int)):
                #
                visibility = 1
                y, x = np.where(segmentation_mask ==
                                int(id_keys_map[obj_name]))
                bounding_box = [int(min(x)), int(max(x)),
                                height - int(max(y)), height - int(min(y))]
            else:
                visibility = 0
        ...
        dict_out['objects'].append({
            ...
            'visii_object_id': int(obj.get_id()),
            'local_cuboid': cuboid,
    ...
    if scene_uuid is not None:
        dict_out['scene_uuid'] = scene_uuid
...
def material_from_json(path_json, randomize=False):
    ...
    if visii.texture.get(data['color']):
        ...
        mat.set_base_color_texture(
            visii.texture.create_from_file(data['color'], data['color']))
    ...
    if 'gloss' in data:
        if visii.texture.get(data['gloss']):
            ...
            roughness_tex = visii.texture.create_from_data(
                ...
                im_r.shape[1],
                im_r.reshape(im_r.shape[0] * im_r.shape[1],
                             4).astype(np.float32).flatten()
            )
            mat.set_roughness_texture(
                visii.texture.create_from_file(data['gloss'], roughness_tex))

    if "normal" in data:
        if visii.texture.get(data['normal']):
            mat.set_normal_texture(visii.texture.get(data['normal']))
        else:
            mat.set_normal_texture(visii.texture.create_from_file(
                data['normal'], data['normal'], linear=True))
    ...
    for file in files:
        ...
        if 'color' in file.lower():
            if visii.texture.get(name_file_visii):
                ...
                mat.set_base_color_texture(
                    visii.texture.create_from_file(name_file_visii, file))
        if 'normal' in file.lower():
            if visii.texture.get(name_file_visii):
                mat.set_normal_map_texture(visii.texture.get(name_file_visii))
            else:
                mat.set_normal_map_texture(visii.texture.create_from_file(
                    name_file_visii, file, linear=True))
        if 'rough' in file.lower():
            if visii.texture.get(name_file_visii):
                mat.set_roughness_texture(visii.texture.get(name_file_visii))
            else:
                mat.set_roughness_texture(visii.texture.create_from_file(
                    name_file_visii, file, linear=True))
        if 'metal' in file.lower():
            if visii.texture.get(name_file_visii):
                mat.set_metallic_texture(visii.texture.get(name_file_visii))
            else:
                mat.set_metallic_texture(visii.texture.create_from_file(
                    name_file_visii, file, linear=True))
